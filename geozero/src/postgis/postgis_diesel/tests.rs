...
fn coordinate_dimensions_from_ewkb() {
    match roundtrip::<_, PointZ>(&PointZ::new(1.0, 2.0, 3.0, SRID)) {
        GeometryContainer::Point(p) => assert_eq!((p.x, p.y, p.z, p.srid), (1.0, 2.0, 3.0, SRID)),
        other => panic!("Expected PointZ, got {other:?}"),
    }

    match roundtrip::<_, PointM>(&PointM::new(1.0, 2.0, 3.0, SRID)) {
        GeometryContainer::Point(p) => assert_eq!((p.x, p.y, p.m, p.srid), (1.0, 2.0, 3.0, SRID)),
...
mod geojson {
    ...
    /// The impls have to drive any [`crate::GeomProcessor`], not just the WKT and WKB writers.
    #[test]
